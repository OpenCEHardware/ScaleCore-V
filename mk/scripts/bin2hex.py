...
# https://gist.githubusercontent.com/pavel-a/89d71b3aba9d7a9e6f8a61d728b08a8e/raw/7531987ba59997cddc3bd31fca8c69dd43fdafe2/bin2hexp.py
...
def convertBinaryToHex(binaryPath, hexPath, start_addr = 0, noEndRecord = False, ignoreErasedRecords = True):
    ...
    with open(binaryPath, 'rb') as binaryFile:
        data = binaryFile.read(HEX_LINE_LEN)
        while len(data) == HEX_LINE_LEN:
            data = list(data)[::-1]
            if address == 0 :
                hexFile.write(ConstructRecord(0x04, 0x0000, addr_high.to_bytes(2, 'big')))
            ...
            for val in data:
                if val != 0xFF or not ignoreErasedRecords:
                    hexFile.write(ConstructRecord(0x00, address - 1, data))
                    break
            if address > 0xFFFF:
                addr_high += 1
                address -= 0x10000
        ...
        if len(data):
            data = list(data)
            data.extend([0] * (HEX_LINE_LEN - len(data)))
            data = data[::-1]
            for val in data:
                if val != 0xFF or not ignoreErasedRecords:
